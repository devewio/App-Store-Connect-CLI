    ...
    "submit": {
      ...
      "steps": [
        ...
        },
        {
          ...
          "if": "BUILD_ID",
          "run": "asc release run --app $APP_ID --version $VERSION --build $BUILD_ID --platform $PLATFORM --metadata-dir ./metadata/version/$VERSION --confirm"
        },
        {
          "name": "check_status",
  ...
  "workflows": {
    ...
    "submit_for_review": {
      ...
      "steps": [
        ...
        {
          "name": "submit",
          "run": "SUBMISSION_ID=$(asc review submissions-create --app $APP_ID --platform $PLATFORM --output json | jq -r '.data.id') && asc review items-add --submission $SUBMISSION_ID --item-type appStoreVersions --item-id $VERSION_ID && asc review submissions-submit --id $SUBMISSION_ID --confirm"
        }
      ]
    }
  }
