...
Automate the complete App Store submission process, from preparing metadata through monitoring review status. Use `asc release run` for the canonical end-to-end App Store path, then use `asc submit status` or `asc submit cancel` to monitor and manage the review lifecycle.
...
6. A metadata directory such as `./metadata/version/1.0.0` if you plan to use `asc release run`
...
<Steps>
  ...
  <Step title="Submit for review">
    ...
    Preview the plan before mutating anything:
...
# 4. Create and submit the review submission
...
Submit preflight failed: submission-blocking localization fields are missing:
  ...
  - es-ES: whats-new
Fix these with `asc apps info edit` (optionally using `--copy-from-locale`) before retrying `asc release run`.
```
