...
> Publish with `release run`, then use `validate` or `submit` for follow-up checks
...
Submit preflight failed: submission-blocking localization fields are missing:
  ...
  - es-ES: whats-new
Fix these with `asc apps info edit` (optionally using `--copy-from-locale`) before retrying `asc validate` or `asc release run`.
```
